/*
 * GARDENS EXPERIENCES — Dedicated experiences sub-page
 * Uses shared Costa Rica excursions data with Gardens palette
 */
...
const CDN = {
  heroVideo: `${CDN_BASE}/hanging-bridges-horizontal_0bf48537.mp4`,
  landscapeImg: `${CDN_BASE}/bird-watching-horizontal_abd70271.mp4`,
};

const sectionPadding = "py-20 md:py-32 px-6 md:px-10";
const maxW = "max-w-[1200px] mx-auto";

const property = properties.find((p) => p.id === "gardens")!;
const categories = property.excursionCategories.filter((c) => c.id !== "all");
const excursions = property.excursions;

export default function GardensExperiences() {
  const [activeCategory, setActiveCategory] = useState(categories[0]?.id ?? "");
  const filtered = excursions.filter((e) => e.category === activeCategory);
      ...
      <section className={sectionPadding} style={{ backgroundColor: PALETTE.gradientEnd }}>
        <div className={maxW}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
            {filtered.map((exp, i) => (
              <AnimateOnScroll key={exp.name} variants={fadeUp} delay={i * 0.08}>
                <div
