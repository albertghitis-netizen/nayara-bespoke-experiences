...
export default function Berto() {
  const [activeView, setActiveView] = useState<CategoryId | "calendar" | "home">("home");
  const [menuOpen, setMenuOpen] = useState(false);
  const [entries, setEntries] = useState<CalendarEntry[]>(() => {
    const saved = localStorage.getItem("berto-entries");
    return saved ? JSON.parse(saved) : [];
  });

  // Persist entries
  useEffect(() => {
    localStorage.setItem("berto-entries", JSON.stringify(entries));
  }, [entries]);
      ...
      entries.forEach((entry) => {
        ...
        if (diff >= -15000 && diff <= 15000) {
          ...
          if (Notification.permission === "granted") {
            new Notification(`Berto Reminder`, {
