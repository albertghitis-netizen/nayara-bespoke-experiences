...
function HeroSection() {
  ...
  const heroSrc = isMobile ? CDN.heroMobile : CDN.heroDesktop;
  ...
  return (
    <section ref={ref} className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 h-full flex flex-col justify-end items-center pb-10 md:pb-16 px-6 md:px-10">
        <WordReveal
          text="Atacama Desert Oasis Under the Stars"
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-16 items-start mb-16">
          ...
          <SlideFromRight delay={0.2} className="md:flex-1">
            <div ref={imgRef} className="overflow-hidden rounded-lg">
              <motion.img src={CDN.s1} alt="Rainbow Valley landscape at Alto Atacama" className="w-full object-cover" style={{ aspectRatio: "3/4", y: imgY }} loading="lazy" />
            </div>
          </SlideFromRight>
        </div>
        <KenBurnsImage src={CDN.s2} alt="Nayara Alto Atacama resort with desert mountains" aspect="16/9" />
...
/* ═══ ROOMS — Slider with placeholder room types (TBD) ═══ */
function RoomsSection() {
  const roomTypes = [
    { id: "atacama-room-1", name: "", image: CDN.s1 },
    { id: "atacama-room-2", name: "", image: CDN.s2 },
  ];
  return (
    <section id="rooms" className={sectionPadding} style={{ background: 'linear-gradient(180deg, #e3dfd2 0%, #f0ece3 15%, #f5f2eb 50%, #f0ece3 85%, #e3dfd2 100%)' }}>
      <div className={maxW}>
        <ScaleIn>
          <RoomsSlider rooms={roomTypes} title="Desert Sanctuaries" subtitle="Rooms & Suites" />
        </ScaleIn>
      </div>
    </section>
  );
