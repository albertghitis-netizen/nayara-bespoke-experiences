    ...
    <section className="py-8 px-6 md:px-10">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <span className="text-[#3B2B26]/30 text-[10px] tracking-[0.2em] mr-1" style={{ ...body, fontWeight: 600 }}>Destination</span>
          {DESTINATIONS.map((d) => (
            <button
              key={d}
              ...
              className={`px-3 py-1.5 text-[11px] tracking-[0.1em] rounded-full border transition-all duration-300 ${
                activeDestination === d
                  ...
                  : "bg-transparent text-[#5a4a3a]/50 border-[#3B2B26]/12 hover:border-[#3B2B26]/30 hover:text-[#3B2B26]"
              }`}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[#3B2B26]/30 text-[10px] tracking-[0.2em] mr-1" style={{ ...body, fontWeight: 600 }}>Topic</span>
          {PILLARS.map((p) => (
            <button
              key={p}
        ...
        <a href={post.url} target="_blank" rel="noopener noreferrer" className="group block">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 overflow-hidden bg-white/60 rounded-xl">
            <div className="relative aspect-[16/10] lg:aspect-auto overflow-hidden">
              <img src={post.image} alt={post.title} className="w-full h-full object-cover transition-transform duration-500 ease-out group-hover:scale-110" decoding="async" loading="eager" />
            ...
            <div className="flex flex-col justify-center p-8 md:p-12 lg:p-14">
              <div className="flex items-center gap-3 mb-4">
                <span className="text-[#3B2B26]/30 text-[10px] tracking-[0.2em]" style={{ ...body, fontWeight: 500 }}>{post.pillar}</span>
                <span className="w-px h-3 bg-[#3B2B26]/10" />
                <span className="text-[#3B2B26]/30 text-[10px] tracking-[0.2em]" style={{ ...body, fontWeight: 500 }}>{post.destination}</span>
              </div>
              <h2 className="text-[#3B2B26] text-2xl md:text-3xl leading-[1.1] group-hover:text-[#5a4a3a] transition-colors" style={heading}>{post.title}</h2>
              <p className="text-[#4B4A4A]/55 text-[14px] leading-relaxed mt-4 line-clamp-3" style={body}>{post.excerpt}</p>
              <div className="mt-6 flex items-center gap-2">
                <span className="text-[#3B2B26]/40 text-[10px] tracking-[0.2em] group-hover:text-[#3B2B26] transition-colors" style={{ ...body, fontWeight: 500 }}>Read Story</span>
                <ExternalLink className="w-3 h-3 text-[#3B2B26]/30 group-hover:text-[#3B2B26] transition-colors" />
              </div>
            </div>
          </div>
        </a>
      ...
      <div className="max-w-[1200px] mx-auto">
        {posts.length === 0 && totalFiltered <= 1 ? (
          <div className="text-center py-16">
            <p className="text-[#3B2B26]/30 text-[14px]" style={body}>No stories match the current filters.</p>
          </div>
        ) : (
          <>
            ...
            {hasMore && !showAll && (
              <div className="flex justify-center mt-10">
                <button onClick={onShowAll} className="flex items-center gap-2 text-[11px] tracking-[0.15em] text-[#3B2B26]/40 hover:text-[#3B2B26] transition-colors py-3 px-6 border border-[#3B2B26]/15 rounded-full hover:border-[#3B2B26]/30" style={{ ...body, fontWeight: 500 }}>
                  View All Stories
  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-stone-100 rounded-lg mb-4">
        <img src={post.image} alt={post.title} className="w-full h-full object-cover transition-transform duration-500 ease-out group-hover:scale-110" decoding="async" loading="lazy" />
      </div>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[#3B2B26]/25 text-[9px] tracking-[0.15em]" style={{ ...body, fontWeight: 500 }}>{post.destination}</span>
      </div>
      <h3 className="text-[#3B2B26] text-lg leading-[1.2] group-hover:text-[#5a4a3a] transition-colors" style={heading}>{post.title}</h3>
      {post.excerpt && (
