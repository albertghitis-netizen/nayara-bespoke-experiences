/*
 * TENTED CAMP WELLNESS — Dedicated wellness sub-page
...
const CDN = {
  heroVideo: `${CDN_BASE}/hot-springs-horizontal_2508b725.mp4`,
};

const sectionPadding = "py-20 md:py-32 px-6 md:px-10";
const maxW = "max-w-[1200px] mx-auto";

export default function TentedCampWellness() {
  return (
      ...
      <div className="relative w-full h-[55vh]">
        ...
        <div className="absolute bottom-8 md:bottom-12 left-6 md:left-10 z-10">
          <TextReveal as="h1" delay={0.2}>
            ...
            >
              {tentedCamp.theme.spaHeadline.replace("\n", " ")}
            </span>
          </TextReveal>
          <motion.p
            initial={{ opacity: 0 }}
...
function WellnessContent() {
  ...
  const categories = tentedCamp.spaCategories.filter(c => c.id !== "all");
  const [activeCategory, setActiveCategory] = useState(categories[0]?.id || "massage");
  const filtered = tentedCamp.treatments.filter((t: Treatment) => t.category === activeCategory);
              ...
              {categories.map((cat: { id: string; label: string }) => (
                <button
                  ...
                  style={{
                    ...
                    color: activeCategory === cat.id ? "#F5F7F4" : PALETTE.textSecondary,
